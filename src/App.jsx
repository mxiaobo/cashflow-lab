...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    if (pin.length < 4) { setError("至少4位"); return; }
    setLoading(true);
    setError("");
    try {
      ...
      if (!cloud) {
        ...
        try { await cloudSave(pin, d); } catch {}
      }
      onLogin(pin, d);
    } catch (err) {
      // Offline fallback: use local data
      console.warn("Cloud unavailable, using local:", err);
      savePin(pin);
      const d = merge(localLoad());
      onLogin(pin, d);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, fontFamily: sans, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ width: 56, height: 56, borderRadius: 18, background: "linear-gradient(135deg,#D4A373,#B08968)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, marginBottom: 20 }}>🐷</div>
      <div style={{ fontSize: 22, fontWeight: 700, color: T1, marginBottom: 6 }}>小博现金流实验室</div>
      <div style={{ fontSize: 13, color: T2, marginBottom: 32 }}>输入 PIN 码登录，数据云端同步</div>
      ...
      <div style={{ width: "100%", maxWidth: 300 }}>
        <input
          ...
          placeholder="输入 4-8 位数字 PIN"
          style={{ ...inp, textAlign: "center", fontSize: 22, fontFamily: mono, letterSpacing: 8, padding: "16px 20px", marginBottom: 16 }}
        />
        {error && <div style={{ color: "#BC4749", fontSize: 13, textAlign: "center", marginBottom: 12 }}>{error}</div>}
        <button onClick={handleLogin} disabled={loading} style={{
          width: "100%", padding: 16, borderRadius: 14, border: "none",
          background: loading ? "#F5F1EC" : ACC, color: loading ? T2 : "#fff",
          fontSize: 16, fontWeight: 600, cursor: "pointer", transition: "all 0.2s",
        }}>{loading ? "连接中..." : "登录"}</button>
      </div>

      <div style={{ fontSize: 11, color: T2, marginTop: 32, textAlign: "center", lineHeight: 1.8 }}>
        首次输入即注册，PIN 码是你的唯一身份标识<br />
        同一 PIN → 任何设备 → 同一份数据
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Load versions when on "me" tab
  useEffect(() => {
    if (tab === "me" && pin && loggedIn) {
      cloudGetVersions(pin).then(setVersions).catch(() => {});
    }
  }, [tab, pin, loggedIn]);

  const showToast = (msg) => { setToast(msg); setTimeout(() => setToast(""), 2000); };

  // Save: local instantly + cloud debounced
  const save = useCallback((d) => {
    setData(d);
    localSave(d);
    // Debounced cloud sync (wait 2s after last edit)
    if (syncTimer.current) clearTimeout(syncTimer.current);
    syncTimer.current = setTimeout(async () => {
      if (!pin) return;
      setSyncing(true);
      try { await cloudSave(pin, d); } catch (err) { console.warn("Cloud sync failed:", err); }
      setSyncing(false);
    }, 2000);
  }, [pin]);

  const handleLogin = (p, d) => {
    setPin(p);
    setData(d);
    setLoggedIn(true);
  };
  ...
  const handleRestoreVersion = async (versionId, label) => {
    if (!confirm(`确认恢复到此版本？\n${label}\n当前数据会被覆盖。`)) return;
    try {
      const restored = await cloudRestoreVersion(pin, versionId);
      if (restored) {
        save(merge(restored));
        showToast("已恢复 ✓");
      }
  ...
  // Loading
  if (!data) return (
    <div style={{ minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: sans, color: ACC }}>
      <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet" />
      加载中...
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, fontFamily: sans, color: T1, paddingBottom: 90 }}>
      ...
      {/* Sync indicator */}
      {syncing && (
        <div style={{ position: "fixed", top: 12, right: 16, background: "#fff", padding: "6px 14px", borderRadius: 20, fontSize: 12, color: T2, boxShadow: "0 2px 10px rgba(0,0,0,0.08)", zIndex: 200, display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: 3, background: "#34C759", animation: "pulse 1s infinite" }} />
          同步中
        </div>
      )}
        ...
        <div style={{ padding: "0 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "24px 4px 20px" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: T2, marginTop: 1 }}>PIN: {pin.replace(/./g, "•")}　·　云端已同步</div>
            </div>
          </div>
          ...
          <div style={{ ...card, padding: 0 }}>
            ...
            <div onClick={async () => {
              setSyncing(true);
              try { await cloudSave(pin, data); showToast("同步成功 ✓"); } catch { showToast("同步失败"); }
              setSyncing(false);
            }} style={{ padding: "16px 20px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 20 }}>☁️</span>
                <span style={{ fontSize: 15 }}>手动同步到云端</span>
              </div>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={T2} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
            </div>
          </div>
          ...
          {versions.length > 0 && <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20, paddingLeft: 4, paddingRight: 4, marginBottom: 12 }}>
              <span style={{ fontSize: 13, color: T2 }}>版本历史</span>
            ...
            <div style={{ ...card, padding: 0, maxHeight: 320, overflowY: "auto" }}>
              {versions.map((v, i) => {
                ...
                return (
                  <div key={v.id} onClick={() => handleRestoreVersion(v.id, label)} style={{
          ...
          <div style={{ ...card }}>
            ...
            <button onClick={async () => {
              ...
              setPushing(true);
              try { await pushToSheets(data); showToast("同步成功 ✓"); } catch (err) { showToast("同步失败: " + err.message); }
              setPushing(false);
            }} style={{
              width: "100%", padding: 14, borderRadius: 14, border: "none",
          ...
          <button onClick={() => { if (confirm("确认退出登录？本机缓存将清除。")) handleLogout(); }} style={{
            width: "100%", padding: 14, borderRadius: 14, border: "1px solid #f0ebe3",
            background: "transparent", color: "#BC4749", fontSize: 15, fontWeight: 500, cursor: "pointer", marginTop: 8,
          }}>退出登录</button>
        </div>
      )}
