...
.month-picker-wrap{position:relative;display:inline-block;margin-bottom:16px;}
.month-picker-btn{display:flex;align-items:center;gap:6px;}
.month-panel{position:absolute;top:calc(100% + 6px);left:0;background:var(--card);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.12);border:1px solid var(--border);padding:16px;z-index:50;width:260px;}
...
.month-panel-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;}
.mp-cell{padding:10px 0;border-radius:6px;text-align:center;cursor:pointer;transition:all 0.15s;font-size:13px;font-weight:500;color:var(--text2);position:relative;}
    ...
    <div>
      ...
      <div className="page">
        ...
        <div className="card card-header">
          ...
          <div className="kpi-row">
            <div className="kpi-cell">
              ...
              {bestSrc >= 0 && srcYTD[bestSrc] > 0 && <div className="k-sub">最大来源: {srcName(bestSrc)}</div>}
            </div>
            <div className="kpi-cell">
        ...
        <div className="month-picker-wrap">
          <button className="btn month-picker-btn" onClick={() => setShowMonthPicker(!showMonthPicker)}>
            📅 {MO[editMonth]}
        ...
        <div className="cols">
          ...
          <div className="card">
            <div className="label">{MO[editMonth]} {secName("cashflow")}</div>
            <table>
              <thead><tr>
                <th className="seq">#</th>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* DCA */}
            <div className="card">
              <div className="label">{MO[editMonth]} {secName("dca")}</div>
              <table>
                <thead><tr>
            ...
            {/* Discipline */}
            <div className="card">
              <div className="label">{secName("discipline")}</div>
              {RULES.map((rule, i) => {
                const count = data.violations.filter((v) => v.rule === i).length;
                return (
                  <div className="rule-row" key={i}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: count > 0 ? "var(--down)" : "var(--text1)" }}>{ruleName(i)}</span>
        ...
        <div className="card">
          <div className="label">{secName("overview")}</div>
            ...
            <table style={{ minWidth: 700 }}>
              ...
              <tbody>
                ...
                <tr className="total-row">
                  ...
                  <td style={{ fontWeight: 700, color: "var(--up)", background: "var(--up-bg)" }}>{F(ytd)}</td>
                </tr>
                <tr>
        ...
        {/* ===== PRINCIPAL TABLE ===== */}
        <div className="card">
          <div className="label">{secName("principal")}</div>
          <table>
            <thead><tr>
              <th className="seq">#</th>
              <th style={{ textAlign: "left" }}>来源</th>
      ...
      {modal === "settings" && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="label">现金流来源名称</div>
